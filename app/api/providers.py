...
def get_provider_service(db: Annotated[Session, Depends(get_db)]) -> ProviderService:
...
def get_provider_models(provider_id: int, service: ProviderServiceDep):
    return service.get_provider_models(provider_id=provider_id)


@router.post('/sync/{provider_id}', response_model=str)
def sync_providers(provider_id: int, service: ProviderServiceDep):
